...
## Summary
The flake packages the launcher script and support files (entrypoint.sh, init-firewall.sh) into a derivation, substitutes the share directory path at build time, and declares all runtime dependencies so `nix run` works on a fresh machine with only Nix installed. The flake also exports a `container-image` output for building the OCI image from Nix.
