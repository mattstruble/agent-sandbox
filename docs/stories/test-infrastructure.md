...
- [ ] Image availability is checked by verifying `agent-sandbox:<version>` exists in the local image store; the Makefile provides an `ensure-image` target that loads the Nix-built image if missing.
